...
`index.html` не содержит чата или поля сообщений. Он выстроен как единая прокручиваемая история: первый импульс, формирование структуры, текущая версия NEIVUM 0.1, PULSAR, архитектура развития, ранний доступ и OrivantAI.
...
- переход 600–1000 мс в приложение — `bindAppTransitions()` в `neivum-assets/core.js`.
...
Фирменная визуализация находится в классе `PulsarField` файла `site.js`.

- геометрия колец и интерференция — методы построения слоёв;
